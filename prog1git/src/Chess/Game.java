    ...
    private static void printPlayfield(int[][] playfield, Figure[] whitePawns, Figure[] blackPawns, Figure[] whiteKing) {
        ...
        for (int cnt = 0; cnt < playfield.length; cnt++) {
            for (int i = 0; i < playfield.length; i++) {
                ...
                if (i == whitePawns[list].getXPosition() && cnt == whitePawns[list].getYPosition()) {
                    System.out.print(ANSI_RED);
                    System.out.print(" W ");
                    System.out.print(ANSI_RESET);
                } else {
                    if (i == blackPawns[list].getXPosition() && cnt == blackPawns[list].getYPosition()) {
                        System.out.print(ANSI_CYAN);
                        System.out.print(" B ");
                        System.out.print(ANSI_RESET);
                    } else {
                        if (list == 0) {
                            if (i == whiteKing[list].getXPosition() && cnt == whiteKing[list].getYPosition()) {
                                System.out.print(ANSI_CYAN);
                                System.out.print(" K ");
                                System.out.print(ANSI_RESET);
                            } else {
